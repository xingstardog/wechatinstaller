  ...
  var _require = function (modId) {
    ...
    return function (requireId) {
      ...
      try {
        ...
        // 在这里判断是否允许 require
        if (mb.env.platform === 'devtools' && __wxConfig.subPackages) {
...
(function (global) {
    ...
    wx.setPreferredFramesPerSecond = function(fps) {
        if (mb.setPreferredFps) {
            mb.setPreferredFps(fps)
        }
    }
    ...
    wx.getSystemInfo = function (obj) {
        ...
        return invokeAsync("getSystemInfoAsync", {}, cb)
    }

    var screenCanvas = new mb.ScreenCanvas();
//    screenCanvas.setTouchableRects([]);
    var hasCreateScreenCanvas = false;

    wx.createCanvas = function () {
        if (!hasCreateScreenCanvas) {
            hasCreateScreenCanvas = true;
            return screenCanvas;
        } else {
            return new mb.OffscreenCanvas();
        }
    }

    wx.loadFont = function(arg) {
        mb.loadFont(arg)
    }

    var originImage = global.Image;
    wx.createImage = function () {
        return new originImage()
    }
    ...
    wx.onTouchStart = function (callback) {
        screenCanvas.ontouchstart = callback;
    }

    wx.onTouchMove = function (callback) {
        screenCanvas.ontouchmove = callback;
    }

    wx.onTouchEnd = function (callback) {
        screenCanvas.ontouchend = callback;
    }

    wx.onTouchCancel = function (callback) {
        screenCanvas.ontouchcancel = callback;
    }
    ...
    wx.env = {
        ...
        CLIENT_VERSION: mb.clientVersion,
    ...
    wx.getFileSystemManager = function () {
        return {
            readFile: function (args = {}) {
                ...
                JSBridge.invoke("readFile", arg, cb);
            },
            readdir: function (args) {
                ...
                JSBridge.invoke("readdir", arg, args.success)
            },
            ...
            statSync: function (path, recursive = false) {
                ...
                JSBridge.invoke("statSync", arg, res => {
                    if (res && res.errMsg) {
                        const name = 'statSync'
            ...
            saveFile: function (args) {
                ...
                JSBridge.invoke("saveFile", arg, args.success)
            },
            readdirSync: function (dirPath) {
                ...
                JSBridge.invoke("readdirSync", arg, res => {
                    if (res && res.files) {
                        files = res.files
                    } else {
                        errMsg = res.errMsg;
            ...
            unlinkSync: function (filePath) {
                ...
                JSBridge.invoke('unlinkSync', arg, res => {
                    if (res && res.errMsg) {
            ...
            mkdirSync: function (dirPath, recursive = false) {
                ...
                JSBridge.invoke('mkdirSync', arg, res => {
                    if (res && res.errMsg) {
            ...
            unzip: function (args) {
                ...
                JSBridge.invoke("unzip", arg, args.success)
            }
        }
    }
    ...
    wx.onWindowResize = function(callback) {
        JSBridge.on("OnWindowResize", function(data) {
            if (typeof callback === 'function') {
                console.log("window resize", data);
                callback(data.viewId, data.width, data.height);
            }
        })
    }
